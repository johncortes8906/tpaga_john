...
require_once(dirname(__FILE__).'/ILinqProvider.php');
...
class PHPLinq_Initiator {
	...
	public static function registerProvider($class = null, $reference = null) {
		if (!is_null($class) && $class != '') {
			...
			if (is_null($reference) || $reference == '') {
				$reference = str_replace('PHPLinq_', '', $class);
				$reference .= '.php';
			}
	...
	public function in($source) {
		...
		// No provider found...
		throw new PHPLinq_Exception("No valid PHPLinq_ILinqProvider found for the specified data source." . json_encode($source) . json_encode($_registeredProviders));
	}
}
