...
class IqHandler implements Handler
{
    ...
    public function Process()
    {
        if ($this->node->getChild('query') != null) {
            $nodeId = $this->parent->getNodeId();


            if (isset($nodeId['privacy']) && ($nodeId['privacy'] == $this->node->getAttribute('id'))) {
    ...
    $failedNumbers = array();
        ...
        $groupList = array();
            ...
            $nodeId = $this->parent->getNodeId();
            if (isset($nodeId['groupcreate']) && ($nodeId['groupcreate'] == $this->node->getAttribute('id'))) {
            ...
            if (isset($nodeId['leavegroup']) && ($nodeId['leavegroup'] == $this->node->getAttribute('id'))) {
            ...
            if (isset($nodeId['getgroups']) && ($nodeId['getgroups'] == $this->node->getAttribute('id'))) {
            ...
            if (isset($nodeId['get_groupv2_info']) && ($nodeId['get_groupv2_info'] == $this->node->getAttribute('id'))) {
        ...
        if (isset($nodeId['get_lists']) && ($nodeId['get_lists'] == $this->node->getAttribute('id'))) {
        ...
        if (isset($nodeId['cipherKeys']) && ($nodeId['cipherKeys'] == $this->node->getAttribute('id'))) {
            ...
            foreach ($users as $user) {
                ...
                $pendingNones = $this->parent->getPendingNodes();
                if (isset($pendingNones[ExtractNumber($jid)])) {
                    foreach ($pendingNones[ExtractNumber($jid)] as $pendingNode) {
