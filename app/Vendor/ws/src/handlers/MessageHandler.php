...
class MessageHandler implements Handler
{
    ...
    if ($node->getChild('enc') != null) {
        if (extension_loaded('curve25519') && extension_loaded('protobuf')) {
            $ack = new ProtocolNode('ack', array('to' => $node->getAttribute('from'), 'class' => 'message', 'id' => $node->getAttribute('id'), 't' => $node->getAttribute('t')), null, null);
            ...
            case 'media':
              ...
              switch ($node->getChild('enc')->getAttribute('mediatype')) {
                case 'image':
                  ...
                  $hkdfv3 = new HKDFv3();
                  $keys = $hkdfv3->deriveSecrets($image->getRefKey(), hex2bin('576861747341707020496d616765204b657973'), 112);
                  $iv = substr($keys, 0, 16);
                  $keys = substr($keys, 16);
     ...
     //is a group encrypted message
         ...
         foreach ($childs as $child) {
             ...
             $encType = $child->getAttribute('type');
                     ...
                     $skip_unpad = $node->getChild('enc', array('type' => 'skmsg')) == null;
                 ...
                 $plaintext = $this->decryptMessage(array($group_number, $author), $child->getData(), $child->getAttribute('type'), $node->getAttribute('id'), $node->getAttribute('t'));
