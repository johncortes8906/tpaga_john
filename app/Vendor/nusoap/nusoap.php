...
class nusoap_base {
    ...
    * @access	public
	*/
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded',$soapval=false, $wrapit = true) {
				...
				if($valueType=='arraySimple' || preg_match('/^ArrayOf/',$type)){
					...
					if(is_array($val) && count($val)> 0){
						foreach($val as $v){
							...
							if(is_string(key($v))) {
								$xml .= $this->serialize_val($v,key($v),false,false,false,false,$use, false, false);
							} else {
								// $xml .= $this->serialize_val($v,'item',false,false,false,false,$use, false);
							}
					...
					if ($use == 'literal') {
						...
						$this->debug("if $wrapit is false then do nothing because already wrapped");
						if($wrapit) {
